...
class APIService {
    ...
    func queryModel(productName: String) async throws -> String {
        let data = ["question": productName + "-"+LocalizationManager.shared.localizedString(forKey: "language")]
        ...
        matches?.forEach { match in
            if match.numberOfRanges > 1 {
                ...
                if fields.count == 5 {
                    ...
                    let cleanedNumericValueStr = numericValueStr
                        .replacingOccurrences(of: "%", with: "")
                        .components(separatedBy: CharacterSet.letters.union(.whitespaces)).first ?? numericValueStr

                    if let indicatorCode = IndicatorCode.fromString(indicatorCodeStr),
                       let numericValue = Double(cleanedNumericValueStr),
